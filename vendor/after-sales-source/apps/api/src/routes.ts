...
const permissionUpdateSchema = z.object({
  ...
  canApproveInheritance: z.boolean(),
  displayName: z.string().trim().min(2, "姓名至少 2 位").max(50, "姓名过长"),
});
...
const inheritanceRecordSchema = z.object({
  ...
  ["状态"]: z.enum(["待批准", "有效", "已作废"]).optional().default("待批准"),
  ["备注"]: z.string().trim().optional().nullable(),
  ["录入人"]: z.string().trim().optional().nullable(),
  ["激活时间"]: z.string().trim().optional().nullable(),
  ["审批账号"]: z.string().trim().min(1, "请选择审批人"),
});

const inheritancePreviewSchema = z.object({
  sn: z.string().trim().min(1, "SN 不能为空"),
  sourceOrderNo: z.string().trim().min(1, "原订单号不能为空"),
});
...
function buildUserResponse(user: NonNullable<AuthUser>) {
  return {
    ...
    permissions: {
      ...
      canApproveInheritance: user.canApproveInheritance || user.isSuperAdmin,
    },
  };
}
...
async function requireAnyPermission(
  ...
  if (user.isSuperAdmin || permissions.some((permission) => Boolean(user[permission]))) {
    return user;
  }
...
async function resolveApprover(username: string) {
  return prisma.appUser.findFirst({
    where: {
      username,
      status: "APPROVED",
      OR: [{ isSuperAdmin: true }, { canApproveInheritance: true }],
    },
    select: {
      id: true,
      username: true,
...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.put("/api/admin/users/:id", async (request, reply) => {
    ...
    const data = await prisma.appUser.update({
      ...
      data: {
        ...
        canApproveInheritance: parsed.data.canApproveInheritance,
  ...
  app.get("/api/inheritance-records/preview", async (request, reply) => {
    const user = await requireAnyPermission(request, reply, ["canManageInheritance", "canApproveInheritance"]);
    if (!user) {
      return;
    }

    const params = inheritancePreviewSchema.safeParse(request.query);
    if (!params.success) {
      return reply.status(400).send({ success: false, error: params.error.flatten() });
    }

    const result = await warrantyDecisionService.queryWarranty({
      sn: params.data.sn,
      sourceOrderNo: params.data.sourceOrderNo,
    });
    ...
    return reply.send({
      ...
      data: {
        ...
        baseWarrantyDays: result.baseWarrantyDays,
        extraWarrantyDays: result.extraWarrantyDays,
        warrantyEndDate: result.warrantyEndDate,
        decisionStatus: result.decisionStatus,
        decisionMessage: result.decisionMessage,
        entitlementMessage: result.entitlementMessage,
      },
    });
  });
  ...
  app.get("/api/inheritance-approvers", async (request, reply) => {
  ...
  app.get("/api/inheritance-records/my-pending-approvals", async (request, reply) => {
    const user = await requirePermission(request, reply, "canApproveInheritance");
    if (!user) {
      return;
    }
    ...
    const data = await prisma.snInheritanceRecord.findMany({
      where: {
        status: "待批准",
        pendingApproverUsername: user.username,
      },
  ...
  app.post("/api/inheritance-records", async (request, reply) => {
    ...
    const activatedAt = body["激活时间"] ? new Date(body["激活时间"]) : null;

    if (!startDate || !endDate) {
      return reply.status(400).send({ success: false, error: "继承保修开始日或截止日格式不正确" });
    }

    if (endDate < startDate) {
      return reply.status(400).send({ success: false, error: "继承保修截止日不能早于开始日" });
    }

    if (!(await validateOriginalSnExists(body["原序列号"]))) {
      return reply.status(400).send({ success: false, error: "原序列号需存在于设备表或已导入台账中，才能提交继承关系" });
    }

    const approver = await resolveApprover(body["审批账号"]);
    if (!approver) {
      return reply.status(400).send({ success: false, error: "指定审批人不存在，或该账号没有批准继承权限" });
    }
    ...
    const data = await prisma.snInheritanceRecord.create({
      data: {
        ...
        previewActivatedAt: activatedAt && !Number.isNaN(activatedAt.getTime()) ? activatedAt : null,
        ...
        pendingApproverUsername: approver.username,
        pendingApproverName: approver.displayName,
        approvedBy: null,
        approvedAt: null,
      },
    });

    return reply.send({ success: true, message: "继承记录已提交，等待批准后生效", data });
  });
  ...
  app.put("/api/inheritance-records/:id", async (request, reply) => {
    ...
    return reply.send({ success: true, message: "继承记录已更新，等待批准后生效", data });
  });

  app.post("/api/inheritance-records/:id/approve", async (request, reply) => {
    const user = await requirePermission(request, reply, "canApproveInheritance");
    if (!user) {
      return;
    }
    ...
    const existing = await prisma.snInheritanceRecord.findUnique({ where: { id: params.data.id } });
    if (!existing) {
      return reply.status(404).send({ success: false, error: "继承记录不存在" });
    }

    if (!user.isSuperAdmin && existing.pendingApproverUsername && existing.pendingApproverUsername !== user.username) {
      return reply.status(403).send({ success: false, error: "该继承记录已指定其他审批人，请联系对应审批账号处理" });
    }

    const activeCount = await prisma.snInheritanceRecord.count({
      where: { newSn: existing.newSn, status: "有效", id: { not: existing.id } },
    });
    if (activeCount > 0) {
      return reply.status(400).send({ success: false, error: "该新序列号已存在其他有效继承记录，无法重复批准" });
    }
    ...
    const data = await prisma.snInheritanceRecord.update({
      ...
      data: {
        ...
        approvedAt: new Date(),
        pendingApproverUsername: null,
        pendingApproverName: null,
      },
    });

    return reply.send({ success: true, message: "继承记录已批准并生效", data });
  });

  app.post("/api/inheritance-records/:id/reject", async (request, reply) => {
    const user = await requirePermission(request, reply, "canApproveInheritance");
    if (!user) {
      return;
    }
    ...
    return reply.send({ success: true, message: "继承记录已驳回", data });
  });
